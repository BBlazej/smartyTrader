...
### §7.40 — XTB: reducing orders CLOSE trades; never open or flip into a short — ✅ complete [R4-C2, R4-L9]

   - **Problem:** `XApiClient.create_order` always sent `tradeTransaction` with `type=OPEN`. In xAPI `cmd=SELL, type=OPEN` **opens a short** — closing needs `type=CLOSE` with the trade's `order` number. On the XTB demo path every LLM sell, §7.9 auto-exit and close-all opened a short and left the long in place (hedged, double margin), while the local FIFO ledger booked a fictional closing PnL that fed the loss streak and the prompt; the next close-all then sold the short, opening another.
   - **Done ✅ (client):** `XApiClient.get_open_trades(symbol)` (`getTrades` openedOnly → `order`, `cmd`, `volume`, `open_price`, oldest first) and `close_trade(order, symbol, cmd, volume, price=None)` (`type=CLOSE`, the opening `cmd`, partial volume allowed; long closes at the bid / short at the ask when unpriced; rejections come back as `status: rejected`). `_REQUEST_STATUS_MAP` re-grounded to the documented REQUEST_STATUS set (0 error, 1 pending, 3 accepted, 4 rejected) — the invented 2/5/6 codes are gone and unknown codes never read as fills.
   - **Done ✅ (executor):** `XTBExecutor.place_order` reduces first: an order opposite to the symbol's open trades closes them FIFO via `close_trade` (partial if needed) and **never flips** — a remainder beyond the open volume is dropped with a warning. Closed volume flows through the ledger (`on_sell` for longs, `cover` for shorts). With nothing to close a BUY opens a long; a SELL is refused unless `XTBExecutor(allow_short=True)` — this is a long-only agent. Fill prices remain the requested price (find #8 still open).
   - **Tests:** `TestReduceFirstNeverFlip` (flat SELL refused, explicit short opt-in, BUY covers a short, FIFO partial close across trades, oversized close never flips, failed close reported, per-symbol trade lookup) in `test_xtb_executor.py` — the old tests that "closed" through `create_order` were rewritten; `TestCloseTrade` (CLOSE payload, bid-priced long close, rejection as data, open-trade mapping/filter/FIFO, unknown status never a fill) in `test_xtb_client.py`. 683 tests passing.

### §7.41 — Keyed Kraken path: no accidental live trading; honest spot valuation — ✅ complete [R4-C3, R4-L7; amends §7.28, find #1]

   - **Problem:** ccxt's `kraken` (spot) has **no sandbox** — `set_sandbox_mode(True)` raised `TypeError: 'NoneType' object is not iterable`, so the shipped `testnet: true` + `KRAKEN_API_KEY` crashed at startup, and the obvious workaround `testnet: false` traded **production funds** while logging "Kraken testnet executor". Even keyed, spot `get_positions()` returned `[]` and cash was free quote only, so every BUY looked like a loss of its own notional (tripping daily-loss + drawdown and poisoning the persisted peak); SL/TP never fired, close-all did nothing, SELLs weren't clamped. `xtb_execution.account_type: real` likewise needed only a one-word YAML edit.
   - **Done ✅ (no accidental real money):** `ccxt_provider.exchange_has_sandbox()` (offline check) and an actionable `ValueError` from `create_ccxt_provider(testnet=True)` on exchanges without one. The crypto runner's keyed selection: `testnet: true` + sandbox → `<exchange>-sandbox`; `testnet: true` without one (Kraken spot) → **paper**, warned; `testnet: false` → live only with **both** `crypto_agent.live_trading: true` and env `LIVE_TRADING_ACK=I_ACCEPT_REAL_MONEY_RISK` (mode `<exchange>-LIVE`, loud warning) — otherwise paper, warned. The same acknowledgement is required for `xtb_execution.account_type: real`.
   - **Done ✅ (honest spot valuation):** once `fetch_positions` proves unsupported, `KrakenExecutor` stays in spot mode and reports positions from its FIFO ledger (new `PositionTracker.symbols()` / `average_price()`), capped by `fetch_balance()` base totals (ledger-only fallback if that read fails), marked at each cycle's close through the new `update_price` hook, with the entry's SL/TP attached — so valuation, gates, exit enforcement and close-all see real holdings.
   - **Docs:** `docs/API_NOTES.md` no longer claims a Kraken demo URL (and its xAPI notes now carry the §7.40 close semantics and the real REQUEST_STATUS codes); `.env.example` / YAML explain the gates. PLAN §7.28 re-scoped (sandbox exchange or acknowledged minimal live run).
   - **Tests:** runner decision matrix (no-sandbox key → paper with no keyed client built; real sandbox used; `testnet: false` alone → paper; live flag without the exact env phrase → paper; both → `kraken-LIVE` on a non-sandbox client), XTB real account needs the ack, sandbox detection + actionable error against real ccxt classes (offline), `TestSpotPositionsFromLedger` (marked position with levels, a buy no longer reads as a loss, balance cap, balance-failure fallback, no repeated `fetch_positions`, close-all sells spot holdings). 696 tests passing.

### §7.50 — Safe-config overrides that never apply — ✅ complete [R4-M1]

   - **Problem:** three silent no-ops in the override path (same class as find #16). `market_hours` was written into `settings.stocks_agent`, but `StocksAgent` guarded with its construction-time copy `self._market_hours` — never re-read. `interval_minutes` was documented "applies at restart" but never even applied then: the runner scheduled from YAML long before any cycle's override landed. And removal did not revert: `parse_and_apply` only *set* non-None fields, while the dashboard form persisted every displayed field (merged YAML ∘ overrides), so saving the form once pinned all defaults forever — later stricter YAML edits were silently overridden by the stale DB blob.
   - **Done ✅ (single reader):** `StocksAgent` takes an optional live `agent_settings` object — the very `settings.stocks_agent` the control plane mutates (passed by the runner) — and evaluates the market-hours window from it per cycle (`_effective_market_hours`, constructor value as fallback). No second copy in play.
   - **Done ✅ (interval applies, live + at startup):** `AsyncSchedulerManager.reschedule_cycle` re-arms the job (`reschedule_job(trigger="interval", minutes=…)`; fail-soft on a missing job, pinned against real APScheduler semantics). The runner's applier calls it whenever an apply reports `interval_minutes` changed, and stored overrides are now applied *before* scheduling — so a persisted interval governs from the first tick after restart. Removal reverts settings **and** cadence.
   - **Done ✅ (baseline + override):** `Settings` keeps untouched YAML copies (`agent_baselines`, `execution_baseline`; risk already had one). `parse_and_apply` resolves every safe field as *override if present, else baseline* on each apply — an empty/removed blob reverts live objects (settings, `RiskEngine`'s config object, paper fee/slippage, pipeline history limit, agent symbol list) to YAML. The agent applier now fires on override *transitions* including → cleared (`BaseTradingAgent._applied_overrides_raw`; unchanged blobs aren't re-applied, failed applies retry next cycle).
   - **Done ✅ (diff-only persistence):** new `control_config.strip_noop_overrides` drops fields equal to their YAML baseline at save time — both dashboard form and control-API `PUT /api/config` persist only genuine diffs (all-defaults save stores nothing / clears the row), so saving never freezes defaults.
   - **Tests:** `TestRevertOnOverrideRemoval` (settings/risk/pipeline/executor revert; partial override doesn't keep stale fields alive; market-hours + symbols revert incl. agent callback), `TestStripNoopOverrides`, applier transition tests (once-per-blob, → None fires, failed apply retried) in `test_control_plane.py`; `TestRescheduleCycle` (mock + real APScheduler trigger); `tests/unit/test_runner_overrides.py` (stored interval governs scheduling; live change re-arms; removal reverts cadence; `--once` applies overrides without a scheduler); `TestLiveMarketHoursOverride`; form/API no-op-save and clear tests. 717 tests passing.

### §7.52 — Single-instance lock per agent runner — ✅ complete [R4-M3]

   - **Problem:** nothing prevented two runners of the same agent (manual start + systemd, cron `--once` overlapping a live runner, dashboard twin-race when a heartbeat was still fresh or a runner sat in rehydration). Both would share one SQLite DB while trading from **separate** in-memory paper books, exit levels and pending-order state — decisions forked, risk guards halved, the `agent_control` heartbeat ambiguous.
   - **Done ✅:** `core/runner.py::RunnerLock` — an exclusive non-blocking `flock` on `<db-dir>/<component>.runner.lock`, acquired in `run_agent` right after the enabled gate and *before* any storage/component construction; contention raises `RunnerAlreadyRunning` (logged, scripts exit 2 so cron/systemd notices). The kernel releases the lock even when the holder dies abruptly — no stale-pid cleanup; the pid written inside is diagnostics only. Graceful paths release explicitly; `:memory:` databases (tests) skip the lock entirely (unsharable by nature). Complements §7.24's heartbeat twin-check, which stays as the *dashboard-side* heuristic.
   - **Tests:** `tests/unit/test_runner_lock.py` — lock unit semantics (pid written, second handle contends, clean release/reacquire, idempotent release) + runner refusals: scheduled run and `--once` both raise with an externally held lock while asserting nothing was constructed and the DB file never created; sequential runs succeed after graceful release; a disabled agent never touches the lock file. 723 tests passing.

### §7.53 — Drawdown peak re-baseline procedure — ✅ complete [R4-M4]

   - **Problem:** the max-drawdown gate's high-water mark is `MAX(total_value)` over all-time portfolio snapshots, which are never pruned — after a long life (or §7.41's spurious early trips on keyed venues) the latch can sit permanently above reality and freeze every entry, with no documented exit short of hand-editing SQLite.
   - **Done ✅:** audited, CLI-only re-baseline. New `drawdown_resets` table (one row per agent: `baseline_value` + `reset_at`). `Storage.record_drawdown_reset` (upsert + structlog audit line; refuses without an explicit agent) and `Storage.get_effective_peak_equity` — without a reset it is the historical MAX, with one it becomes `max(baseline_value, MAX(total_value since reset_at))`, so pre-reset history stops latching while post-reset gains still raise the peak. The runner seeds from the new method; snapshots are never deleted (history stays auditable). `scripts/rebaseline_drawdown.py --agent crypto [--value X] [--yes]` — dry-run by default, plans (old seed → new baseline) through structlog, defaults the new baseline to the latest snapshot equity, validates before touching the DB. Deliberately **no dashboard button**: loosening a risk guard is never a web-form click (§7.43's rule).
   - **Tests:** `tests/unit/test_drawdown_reset.py` — record/get round-trip + upsert, explicit-agent requirement, no-reset equivalence to historical MAX, reset cuts off pre-`reset_at` snapshots (backdated rows) with the baseline as floor when nothing follows, agent scoping (a crypto reset never touches stocks' latch), and the CLI planner (defaults to latest equity, `--value` override, SystemExit on no-history-without-value and non-positive baselines). 731 tests passing.

### §7.54 — Entry SL/TP geometry validated; optional risk-per-trade sizing — ✅ complete [R4-M5]

   - **Problem:** the entry gate only required `stop_loss is not None`. A BUY with `stop_loss ≥ price` (or `take_profit ≤ price`) was approved and auto-closed by §7.9 exit enforcement on the very next cycle — a guaranteed round trip for double fees; an SL at `0.01` meant unbounded per-trade risk while the stop-distance signal said nothing; sizing ignored where the stop sat.
   - **Done ✅:** `RiskEngine.evaluate(..., current_price=…)` (pipeline and backtester both pass the cycle's mark) extends the stop rule into a geometry check on entries: `stop_loss` must be strictly below price, `take_profit` (when given) strictly above it, and the stop distance `(price − SL)/price` capped by new `risk.max_stop_distance_pct` (default 0.25; boundary inclusive). Sells/closes untouched (§7.47 stays: exits are never gated), and callers without a price keep the old stop-required behavior. Optional sizing: `risk.risk_per_trade_pct` (default 0 = off) caps each BUY in the shared `calculate_quantity` so `(entry − stop) × qty ≤ pct × total_value` — live and replay can't drift (§7.14).
   - **Tests:** `TestEntryGeometry` (stop above/at price rejected; TP at/below price rejected; 0.01-stop rejected via `max_stop_distance_pct`; boundary distance + sane levels approved; geometry skipped without a mark); pipeline end-to-end inverted-stop rejection (gate kills it, no position ever opens); risk-per-trade sizing cap on/off in `TestPositionHeadroomSizing`. Two backtester fixtures moved from 50 %-distance stops to compliant ones. 738 tests passing.

### §7.55 — Refuse to trade without a price — ✅ complete [R4-M6]

   - **Problem:** with an empty candle series the pipeline kept going: `current_price` was `None`, so `planned_notional` skipped the size cap, `calculate_quantity` sized off `signal.stop_loss` (or 1.0) as a price proxy, and the order went out with `price=None` — which `KrakenExecutor` turns into an **unchecked market order**. An LLM verdict on zero market data could buy at whatever the venue felt like.
   - **Done ✅ (two layers):** `DecisionPipeline.run` aborts right after the fetch when `snapshot.candles` is empty — no marking, no indicators, no LLM call, no decision row, result carries `error="No market data (empty candle series)"`. And post-approval the execute step no longer falls back to stop-loss-sized quantities: with no usable mark/quantity it refuses and returns an error instead of calling `place_order` — nothing unpriced ever reaches a venue.
   - **Tests:** `TestNoPriceRefusal` in `test_decision_pipeline.py` — empty candles never reach the LLM (spy on `ask_trade_signal`) nor the executor (order-attempt spy); zero-close candles reach the gate but die at the execution belt with positions still empty. 740 tests passing.

### §7.57 — LLM response parser robustness — ✅ complete [R4-M8]

   - **Problem:** `_parse_signal` only stripped markdown fences and then `json.loads`-ed the whole completion. The configured Qwen3-family model emits `<think>…</think>` blocks (some LM Studio builds put them in `content`), and any leading prose also broke parsing — every attempt failed and the cycle fell back to HOLD. A long reasoning block could also eat the `max_tokens` budget before the JSON, silently.
   - **Done ✅:** `_strip_reasoning` drops closed `<think>`/`<thinking>` blocks, everything before a bare `</think>` (templates that open the block themselves), and a still-open block cut off by `max_tokens` — a JSON *draft* inside reasoning is never traded. `_json_objects` then collects every top-level balanced `{…}` via `JSONDecoder.raw_decode` (braces inside strings can't derail it), and the **last** one that validates as a `TradeSignal` wins (models that correct themselves answer last); no object → `JSONDecodeError`, no valid candidate → the last `ValidationError`, both into the existing retry → HOLD fallback path. `is_fallback` is still stripped from every candidate. A `finish_reason: "length"` completion logs `llm_response_truncated` with the `max_tokens` in force. `use_json_schema` stays opt-in (config comment unchanged) until the local model is confirmed to accept `response_format`.
   - **Tests:** `TestParseSignalReasoningModels` (think-prefixed answer, draft JSON inside think ignored, unopened `</think>` preamble, truncated think never yields its draft, leading/trailing prose, braces in strings, last-valid-wins, trailing non-signal object, no valid signal, fence after think) + truncation-warning test in `TestSeedAndSizeGuard`. 751 tests passing.

### §7.58 — Venue executor restart rehydration — ✅ complete [R4-M9]

   - **Problem:** §7.25's fill replay only ran for executors with `load_portfolio_state` — i.e. paper. After a restart `KrakenExecutor`/`XTBExecutor` had empty FIFO ledgers (closes reported no outcome; Kraken **spot** positions — which come from that ledger since §7.41 — vanished from valuation, gates, exits and close-all) and empty `_exit_levels` (pre-restart SL/TP silently unenforced). `KrakenExecutor._open_orders` was memory-only, so an order open across a restart stayed `pending` in the DB forever. Find #7 / AGENTS.md overstated §7.25 as covering "the FIFO ledgers" generally.
   - **Done ✅:** the replay loop moved into a shared `position_tracker.replay_fills` (paper uses it too) which also returns the live exit-level rule's result (latest buy's SL/TP per symbol, dropped when flat); `FillRecord` gained optional `stop_loss`/`take_profit`. New executor hooks `KrakenExecutor.load_fills` / `XTBExecutor.load_fills` (replace ledger + levels wholesale) and `KrakenExecutor.load_pending_orders(list[PendingOrderRecord])` (re-tracks orders + their symbols for cancel). `core/rehydration.py::rehydrate_venue_executor` (part of `rehydrate_from_storage`, fail-soft per hook) feeds them from the agent-scoped `get_filled_orders` (skipping `paper-…` ids — paper-era fills never happened at the venue) and new `Storage.get_pending_orders`; levels come from the entry decisions via new `Storage.get_exit_levels(decision_ids)` — no schema change, the decision row already stores what the pipeline passed to `place_order`.
   - **Found on the way:** finds #18 (paper↔venue / sandbox→live switches still mix histories) and #19 (cancelled partial fills dropped; stored quantity = requested) → §7.61.
   - **Tests:** `TestVenueRehydration` in `test_rehydration.py` — Kraken spot book + cost basis + latest-entry levels + per-entry attribution after restart with a paper-era fill ignored; flat symbol leaves no lots/levels; other agent's fills not replayed; a pending row re-polled and filled into the ledger with its entry plan; XTB levels re-attached to venue positions and close attributed; storage failures fail-soft; paper executor untouched. 758 tests passing.
