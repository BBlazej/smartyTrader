...
- **Known open gaps (external review 4 → PLAN §7.39–§7.60) — several rules below state intent the code does not yet meet; do not rely on them until the item lands:**
  - **Shared DB is unscoped (§7.39, critical):** `portfolio_snapshots`/`llm_decisions`/`orders` carry no agent column, so with both agents on one DB, restart rehydration, the drawdown peak, daily-loss baseline and loss-streak all mix agents. Any new query on these tables must plan for per-agent filtering.
  - **XTB sells open shorts (§7.40, critical):** `xtb_client.create_order` always uses `type=OPEN`; closes (LLM sell, §7.9 exits, close-all) do not close longs on the venue. Do not enable `xtb_execution` until fixed.
  ...
  - **`max_position_pct` caps each order, not the position (§7.42, critical):** repeated BUYs can pyramid one symbol to ~90% of equity.
  - **Dashboard/control API have no CSRF/Origin/Host protection (§7.43):** any website can POST risk-loosening overrides, close-all or (with `allow_launch`) agent starts; the shipped YAML currently has `dashboard.allow_launch: true` despite the documented false default.
  - **Also open:** post-order persistence is not fail-soft (§7.44); the prompt carries no position/cash context (§7.45); restart double-counts losing round trips into the streak (§7.46); LLM exits are blocked by daily-loss/drawdown/cooldown (§7.47); closes ignore `Position.side` (§7.48); the backtester has candle look-ahead (§7.49); `market_hours`/`interval_minutes` overrides never apply (§7.50 — contrary to "applies at restart" below); LLM fallbacks are not surfaced as errors (§7.51); no single-instance runner lock (§7.52); venue executors (Kraken/XTB) do **not** rehydrate FIFO ledgers, exit levels or pending orders — §7.25 covers paper only (§7.58).
