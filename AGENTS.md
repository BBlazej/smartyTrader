...
- **Known open gaps (external review 4 → PLAN §7.39–§7.60) — several rules below state intent the code does not yet meet; do not rely on them until the item lands:**
  - **Also open:** no single-instance runner lock (§7.52); venue executors (Kraken/XTB) do **not** rehydrate FIFO ledgers, exit levels or pending orders — §7.25 covers paper only (§7.58).
