...
62. **XTB fills booked at the requested price** ⏳ [find #8]
    - `XApiClient.create_order` / `close_trade` return the price they were *called* with, and `XTBExecutor` feeds that into the FIFO ledger. xAPI instant orders fill at market, so cost basis, realized PnL and entry attribution of XTB trades can drift from the venue (harmless while `xtb_execution.enabled: false`).
    - Fix: after the transaction resolves, read the actual fill price (`getTrades` `open_price` for opens; the closed trade's `close_price` for closes, or the transaction status payload) and book that; keep the requested price only as a fallback. Do before enabling XTB execution.
