...
stocks_agent:
  ...
  demo: true
  interval_minutes: 15
  market_hours: "09:00-16:30"   # local wall-clock window (wrap-around windows supported)
  market_timezone: "Europe/Warsaw"   # zone the window is in (so a UTC host stays correct)
  market_holidays: []   # ISO YYYY-MM-DD closure dates (§7.10); weekends are always closed
  symbols:
    - AAPL
    - MSFT
  decision_history_limit: 10   # prior decisions fed back into the prompt (0 = off)
...
risk:
  ...
  min_confidence: 0.6
  enforce_exit_levels: true   # deterministic stop/take-profit closes (§7.9)
...
10. **Weekend/holiday awareness in the stocks market-hours guard** — ✅ **complete** **[R-M6]**
...
11. **Stocks data depth silently disables indicators** — ✅ **complete** **[R-M7]**
