...
**Current state (2026-09-18):** 436 tests passing at ~93% coverage. §7.1–§7.14 and §7.20–§7.23 are complete, and §7.15 P1–P4 (control plane + control API + dashboard pages & config UI) are delivered (see HISTORY.md). Open: §7.15 P5 (Docker packaging), §7.16 XTB OAuth2, §7.17–§7.19 housekeeping.
...
15. **Dashboard + control (Docker WebUI)** 🔨 in progress (P1–P4 done: control table + agent checks + control API + dashboard pages & config UI; P5 remaining: Docker packaging) — design in [ARCHITECTURE.md](ARCHITECTURE.md) — "Data pipeline, storage & dashboard"
   ...
   - **Phased build:**
     ...
     - P3 — dashboard (monitor) — ✅ **done** (`src/dashboard/`: `create_dashboard_app` + pure view-models in `views.py`, Jinja2/HTMX templates). Pages: overview (portfolio cards, uPlot portfolio-value chart fed by `/api/portfolio.json` polling, recent decisions), positions, decisions (+ win-rate / avg-confidence / confidence-histogram stats), health cards. Live refresh via HTMX polling (`/partials/health`) at `dashboard.refresh_seconds`. Standalone entry point `scripts/run_dashboard.py` reads the shared SQLite DB (WAL) as a reader. (Tests: unit for view-models + integration seeded-DB render.)
     - P4 — dashboard (control + config) — ✅ **done** (same app). HTMX Pause/Resume/Close-all buttons post to `POST /control/{agent}/{action}`, which writes the `agent_control` latches **directly** (same repository methods as the control API; running agents pick them up next cycle, so no HTTP coupling and it works with or without `control_api.enabled`). Safe config form (`GET/POST /config/{agent}`) parsed into the nested payload and validated server-side through `validate_overrides_payload` → `SafeConfigOverrides` (`extra="forbid"` — any injected credential-shaped key rejects wholesale, form re-renders with the rejection); accepted values persist to `agent_control.config_override_json`. (Tests: config whitelist rejects credentials.)
