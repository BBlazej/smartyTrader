...
> **This section lists only open work.** Items §7.1–§7.27, §7.29–§7.33, §7.35 and §7.37–§7.58 were completed in 2026-09; their full write-ups live in [HISTORY.md](HISTORY.md) under their original numbers. §7.N identifiers are **never renumbered or reused**.

### Critical / high severity (open)

> Order of work (from `external_4.md` §7; §7.39–§7.58 done — the §4.3 paper clock can start, replay numbers are look-ahead-free and the venue prerequisites §7.40/§7.41/§7.48/§7.58 are in). Nothing critical/high remains open; finds #18/#19 (`nightly_finds.md`) should land before a keyed venue run (§7.28).
...
61. **Venue history hygiene** ⏳ [finds #18, #19 — found while landing §7.58]
    - Paper and venue executors of one agent share its `orders`/`portfolio_snapshots` rows: a venue → paper (or sandbox → live) switch rehydrates the other executor's book/fills (§7.58 only filters `paper-…` fills out of venue replays).
    - Reconciled partial fills of cancelled orders never reach the ledger, and `update_order_status` keeps the *requested* quantity — the §7.58 replay then rebuilds lots from requested sizes.
    - Fix: tag rows with the execution venue and replay per venue; feed cancelled-with-`filled > 0` through the ledger and persist the filled quantity.
