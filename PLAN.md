...
1. **Refresh paper-position prices every cycle** ⏳ **[R-H1]** — `PaperExecutor.update_price()` (`src/execution/paper_executor.py:205`) is never called by production code (tests only), so after entry a position's `current_price` stays frozen at its fill price: `unrealized_pnl` is always 0, `total_value` never moves with the market between trades, and **the daily-loss rule can only ever see fee/trade-flow drift, never actual market moves** — the headline `-daily_loss_limit_pct: 0.02` guard is effectively cosmetic in paper mode. Fix: feed each cycle's snapshot last-close into `update_price(symbol, close)` for open positions (cycle start or `_post_process`), or make `get_positions` take a price map. Highest-leverage single change: honest daily-loss rule + unrealized PnL + usable portfolio snapshots for the dashboard/backtester + prerequisite for §7.9 (SL/TP exits). Add a test asserting `unrealized_pnl != 0` after a simulated price move.

2. **Fix `enabled: false` semantics** ⏳ **[R-H3]** — both runners treat "disabled" as *run one full cycle — including order placement, LLM calls and DB writes — then exit* (`scripts/run_crypto_agent.py:155-161`, `scripts/run_stocks_agent.py:114-120`). A user who disables an agent to be safe still gets trades executed. `enabled: false` should exit without running anything; if single-cycle mode is wanted, add an explicit `run_once` flag rather than overloading `enabled`.

3. **Dependency & test-claims cleanup** ⏳ **[R-M4]** — `pandas-ta` is declared in `pyproject.toml` but never imported anywhere (indicators are hand-rolled; also a numpy-compat risk); `python-dotenv` is declared but never used (replaced by a duplicated hand-rolled `_load_dotenv()` in both scripts); no dev-dependency group exists, so pytest/pytest-asyncio/pytest-cov/ruff are installed ad-hoc and the env isn't reproducible; `AGENTS.md` claims property-based tests via `hypothesis` plus `responses` — neither is installed nor used. Remove or wire each; add `[project.optional-dependencies].dev`; then either add real hypothesis tests for risk-engine invariants or correct `AGENTS.md`. Also give the stocks runner a fail-fast hint when `yfinance` is missing (mirrors the ccxt one) — today it just logs fetch errors every cycle.
...
5. **Close the two no-op risk rules (drawdown; position size at the gate)** ⏳ **[R-H2]** *(absorbs the earlier external-review item "Deferred #2")*
   - **Max drawdown is a stub:** `RiskEngine._check_drawdown` returns `APPROVED` unconditionally and `risk.max_drawdown_pct` is loaded from config but never used; only the daily-loss + consecutive-loss gates are live. **Decision needed before building:** (a) in-memory peak-equity tracker (resets on process restart — simple, but loses the peak across restarts), vs (b) **SQLite-persisted high-water mark** (the review's recommendation; reuse `MAX(total_value)` from `portfolio_snapshots` or a dedicated `peak_equity` column). Recommend **(b)** for cross-restart persistence. Constraint: `RiskEngine` is a pure sync object with no DB access — the peak must be fed in from the agent (like `update_daily_value`) or read from storage. Then wire `_check_drawdown` to `settings.max_drawdown_pct` and add a unit test (peak→trough rejection). *Note: depends on §7.1 — with frozen position prices, portfolio value barely moves and even a correct drawdown rule would rarely fire.*
   - **Position size is never validated at the gate:** `_check_position_size` only rejects when total value ≤ 0; actual sizing happens *after* approval in `DecisionPipeline._calculate_quantity`, so if sizing ever regresses nothing catches it. Move a notional-cap check (`quantity × price ≤ max_position_pct × total_value`) into `evaluate()`.

6. **Real-CCXT integration pass for the keyed Kraken path** ⏳ **[R-H4]** — the executor is only tested against mocks; against real ccxt (installed: 4.5.x) it likely breaks where it matters: `get_cash` does `float(balance)` but real `fetch_free_balance` returns a **dict** → `TypeError` every keyed cycle; Kraken spot via CCXT does not serve `fetch_positions` (empty or `NotSupported`) so max-open-positions and portfolio valuation see nothing on the keyed path; and because the pipeline always passes `price=current_price`, *every* order becomes a limit-at-last-close with no fill reconciliation — status can stay `"pending"` forever and `filled_at` is never set even for `"closed"`. Needs an integration pass against live testnet (or recorded fixtures) before "Kraken testnet mode" is offered.

7. **Persist / rehydrate paper portfolio + risk state across restarts** ⏳ **[R-M1]** — cash/positions (`PaperExecutor`, memory-only) and the daily-loss/consecutive-loss/cooldown trackers are all in-memory, while decisions/orders persist in SQLite. After a restart: cash resets to 100k, open positions vanish (persisted decisions reference trades that no longer exist), and the daily baseline + cooldown reset to zero — silently weakening every stateful guard. Also `initial_cash` is hardcoded (`paper_executor.py:20`, violates "config-driven"); add `execution.initial_cash`. Fix direction: persist executor state in SQLite **or** deterministically rebuild positions/cash from the orders table at startup, and rehydrate risk trackers from recent orders/decisions.

### C. Medium severity

8. **Decision-history quality: attribute outcomes to entry decisions; exclude fallback rows** ⏳ **[R-M2/M3]** *(absorbs the earlier external-review item "Deferred #4")*
