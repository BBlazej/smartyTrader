...
> **This section lists only open work.** Items §7.1–§7.27, §7.29–§7.33, §7.35, §7.37–§7.62 and §7.64 were completed in 2026-09; their full write-ups live in [HISTORY.md](HISTORY.md) under their original numbers. §7.N identifiers are **never renumbered or reused**.
...
> Order of work (2026-09-26, venues decided — **OKX Europe** for crypto, **Saxo** for stocks, CHANGE.md Q1; **crypto first**, ≈ €1,000 budget, free services only): ~~§7.64~~ (done) → §7.65 (per-venue cost model) → §7.28 (OKX demo smoke run) → §7.69 (LLM latency benchmark) → CHANGE.md crypto screener + crypto sleeves → §7.66 (Saxo executor, SIM/paper only) → §7.68. These are the prerequisites of the CHANGE.md proposal (§5) and make paper numbers honest for the §4.3 gates.
